    ...
    <form method="POST" action="{{ route('register') }}" novalidate>
        ...
        <div class="mt-4">
            ...
            <select name="rol" id="rol" class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                <option value="" disabled {{ old('pais') ? '' : 'selected' }} hidden>~~ Seleccione un Rol ~~</option>
                <option value="1" {{ old('rol')=='1' ? 'selected' : '' }}>Developer - Obtener Empleo</option>
                <option value="2" {{ old('rol')=='2' ? 'selected' : '' }}>Recruiter - Publicar Empleos</option>
            </select>
        ...
        <div class="flex items-center justify-between mt-4">
            ...
            <x-primary-button class="ms-4 px-7">
                {{ __('Registrarse') }}
            </x-primary-button>
        </div>
    </form>
